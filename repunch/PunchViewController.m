...
//    [bumpDiagram setBackgroundColor:[UIColor colorWithRed:.5 green:0 blue:0 alpha:.5]];
    ...
    [bumpInfoLabel setText:@"Gently bump the customer's device to redeem punches."];
    ...
    [[BumpClient sharedClient] setDataReceivedBlock:^(BumpChannelID channel, NSData *data) {
        ...
        [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error){
            if (error) {
                ...
                int newPunchCount = [[receivedData objectForKey:@"num_punches"] intValue] +
                ...
                [thisPFPlace saveInBackgroundWithBlock:^(BOOL succeeded, NSError *saveError){
                    if (saveError) {
                        ...
                        UIAlertView *av = [[[UIAlertView alloc] initWithTitle:@"Success" message:[NSString stringWithFormat:@"You received %d punches from %@.",[[receivedData objectForKey:@"num_punches"] intValue],thisPlace.name] delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil] autorelease];
                        [av show];
                    }
                }];
            }

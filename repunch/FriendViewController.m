...
@synthesize friendData, friendTableView, alphabet, reward;
...
- (void)viewDidLoad
{
    ...
    alphabet = [NSArray arrayWithObjects:@"A",@"B",@"C",@"D",@"E",@"F",@"G",@"H",@"I",@"J",@"K",
    ...
    [friendListRequest startWithCompletionHandler:^(FBRequestConnection *connection, id result, NSError *error){
        if (error) {
            ...
            friendData = [result objectForKey:@"data"];
            friendData = (NSMutableArray *)[[friendData sortedArrayUsingDescriptors:[NSArray arrayWithObject:[[[NSSortDescriptor alloc] initWithKey:@"name" ascending:YES] autorelease]]] retain];
